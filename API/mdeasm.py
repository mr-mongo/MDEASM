...
class Workspaces:
    ...
    def __run_discovery_group__(self, disco_name, workspace_name=''):
        if not workspace_name:
            workspace_name = self._default_workspace_name
        if self.__verify_workspace__(workspace_name):
            r = self.__workspace_query_helper__('__run_discovery_group__', method='post', endpoint=f"discoGroups/{disco_name}:run", payload={'a':'b'})
    ...
    def create_discovery_group(self, workspace_name='', disco_template='', disco_custom={}):
            ...
            if self.__verify_workspace__(workspace_name):
                if disco_custom:
                    try:
                        ...
                        payload = {'tier':'advanced', 'frequencyMilliseconds':604800000, 'seeds':seeds, 'names':[], 'excludes':[]}
                    except KeyError:
                ...
                r = self.__workspace_query_helper__('create_discovery_group', method='put', endpoint=f"discoGroups/{disco_name}", payload=payload)
                if self.__run_discovery_group__(disco_name=disco_name, workspace_name=workspace_name) == 204:
